...
/// Represent a `Field` in the schema.
public struct Field {
    ...
    public init(name: String, alias: String? = nil, arguments: [String : InputValue]? = nil, directives: [Directive]? = nil, type: FieldType) {
        self.name = name
        self.alias = alias
...
public indirect enum FieldType {
    ...
    public mutating func merge(_ other: FieldType) throws {
        ...
        case (.list(var lField), .list(let rField)):
            try lField.merge(rField)
            self = .list(lField)
        case (.nonNull(var lField), .nonNull(let rField)):
            try lField.merge(rField)
            self = .nonNull(lField)
        default:
            struct Err: LocalizedError {
                let string: String
                var localizedDescription: String { return self.string }
            }
            throw Err(string: "Cannot merge \(self.stringCaseDescription) with \(other.stringCaseDescription)")
        }
    }
    ...
    public var selectionSet: SelectionSet? {
        ...
        case .object(let selectionSet): return selectionSet
        case .list(let fieldType):      return fieldType.selectionSet
        case .nonNull(let fieldType):   return fieldType.selectionSet
        }
    }
    ...
    public var selectionKind: SelectionKind {
        ...
        case .list(let type):       return type.selectionKind
        case .nonNull(let type):    return type.selectionKind
        }
    }
    
    public var stringCaseDescription: String {
        switch self {
        case .scalar:               return StringCase.scalar.rawValue
        case .object:               return StringCase.object.rawValue
        case .list(let type):       return "\(StringCase.list.rawValue)(\(type.stringCaseDescription))"
        case .nonNull(let type):    return "\(StringCase.nonNull.rawValue)(\(type.stringCaseDescription))"
        }
    }
}
...
public struct FragmentDefinition: AcceptsSelectionSet, AcceptsDirectives, QueryConvertible {
    ...
    public let directives: [Directive]?
    public let selectionSet: SelectionSet
    // TODO: This is not yet in the query builder or generated by code gen. Add it.
    public let possibleTypes: [String]
    
    public var selectionSetDebugName: String {
        return self.name
    }
    
    public init?(name: String, type: String, directives: [Directive]? = nil, selectionSet: SelectionSet, possibleTypes: [String] = []) {
        guard name != "on" else {
            return nil
        }
    ...
    public init?(name: String, type: String, directives: [Directive]? = nil, selectionSet: [FieldRepresenting], possibleTypes: [String] = []) {
        self.init(name: name, type: type, directives: directives, selectionSet: SelectionSet(selectionSet), possibleTypes: possibleTypes)
    }
...
public struct SelectionSet: ExpressibleByArrayLiteral, SelectionSetSerializable, QueryConvertible {
    ...
    public var selections: [Selection] {
        return self.selectionSet.map { $0.1 }
    }
    
    public var selectionSetDebugName: String {
        return self.selectionSet.map { $0.0 }.joined(separator: ", ")
    }
...
public enum Selection: ObjectSerializable, InlineFragmentSerializable, SelectionType {
    ...
    public var kind: SelectionKind {
        switch self {
        case .field(_, _, _, _, type: let type): return type.selectionKind
    ...
    public func serializedSelections() throws -> [String] {
        ...
        case .field(name: _, alias: _, arguments: _, directives: _, type: let type):
            return try type.selectionSet.map { try $0.serializedSelections() } ?? []
        case .fragmentSpread:
            return []
    ...
    public func graphQLString() throws -> String {
        ...
        case .field(_, _, _, _, type: let type):
            func gqlString(fieldType: FieldType) throws -> String {
                switch fieldType {
                case .scalar:                   return try self.serializedWithoutSelectionSet()
                case .object:                   return try objectGraphQLString(for: self)
                case .list(let innerType):      return try gqlString(fieldType: innerType)
                case .nonNull(let innerType):   return try gqlString(fieldType: innerType)
                }
            }
            return try gqlString(fieldType: type)
        case .fragmentSpread(name: let name, directives: let directives):
            return try FragmentSpread(name: name, directives: directives).graphQLString()
